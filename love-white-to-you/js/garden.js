...
Vector.prototype={
	...
	length:function(){
		...
		subtract:function(a){this.x-=a.x;this.y-=a.y;
	return this},
	set:function(a,b){
		this.x=a;
		this.y=b;
...
function Petal(a,f,b,e,c,d){
	...
	Petal.prototype={
		draw:function(){
			...
			a.strokeStyle=this.bloom.c;a.beginPath();
			a.moveTo(e.x,e.y);
	...
	init:function(){
		...
		var b=Garden.randomInt(0,90);
			for(var a=0;a<this.pc;a++){
				...
				Garden.random(Garden.options.petalStretch.min,
				Garden.options.petalStretch.max),
				b+a*c,c,Garden.random(Garden.options.growFactor.min,Garden.options.growFactor.max),this))
			}
	}
};
function Garden(a,b){
...
Garden.prototype={
	...
	createRandomBloom:function(a,b){
		...
		Garden.options.color.gmax,Garden.options.color.bmin,
...
Garden.options={petalCount:{min:8,max:15},
petalStretch:{min:0.1,max:3},
growFactor:{min:0.1,max:1},
bloomRadius:{min:8,max:10},
density:10,growSpeed:1000/60,color:{
	rmin:128,rmax:255,gmin:0,gmax:128,bmin:0,bmax:128,opacity:0.1},tanAngle:60
};
Garden.random=function(b,a){
	...
	Garden.randomrgba=function(i,n,h,m,l,d,k){var c=Math.round(Garden.random(i,n));
